...
function BeverageItem({ name, price }: { name: string; price: string }) {
  return (
    <div className="flex items-baseline justify-between gap-1">
      <span className="font-sans text-[13px] text-[#0e7c6b] font-medium">{name}</span>
  ...
  return (
    <div className="menu-page w-[210mm] h-[297mm] mx-auto relative bg-[#faf8f3] overflow-hidden flex flex-col">
      ...
      <div className="px-7 flex-1 flex flex-col pt-2">
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-5 mb-3">
        ...
        <div className="flex gap-5 items-center">
          <div className="w-[180px] h-[180px] rounded-full overflow-hidden border-2 border-[#d4af37] border-opacity-50 shadow-md flex-shrink-0">
            <Image
          ...
          <div className="flex-1">
            <div className="bg-[#0e7c6b] bg-opacity-5 rounded-lg p-5 border border-[#d4af37] border-opacity-30">
              ...
              <p className="font-sans text-[13px] text-[#0e7c6b] mb-3 font-medium">
                {beerBites.description}
              </p>
              <div className="flex flex-col gap-1.5">
                <p className="font-sans text-[11px] text-[#5a7a6e] font-bold uppercase tracking-wider mb-1">
                  Beer Bites Choices:
                </p>
                {beerBites.choices.map((choice) => (
                  <div key={choice} className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-[#d4af37] rounded-full" />
                    <span className="font-sans text-[13px] text-[#0e7c6b]">{choice}</span>
                  </div>
                ))}
              </div>
            </div>
