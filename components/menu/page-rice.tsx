...
export function PageRice() {
  return (
    <div className="menu-page w-[210mm] h-[297mm] mx-auto relative bg-[#faf8f3] overflow-hidden flex flex-col" data-page-name="rice">
      {/* Corner accents */}
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-1">
          {rice?.items?.map((item) => (
            <div key={item?.name}>
              ...
              {item.description && (
                <p className="font-sans text-[12px] text-[#5a7a6e] leading-snug mt-0.5">
                  {item.description}
                </p>
              )}
            </div>
